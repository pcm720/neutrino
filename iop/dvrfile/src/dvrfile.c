...
// Locally handled ioctl that returns the DVRP-side file descriptor of an open file for Neutrino FHI.
// Used by the EE loader to get the persistent handle for dvrfile_fhi.
#define DVRFILE_IOCTL_GET_FD 0x80
...
int dvrf_df_ioctl2(iomanX_iop_file_t *f, int cmd, void *arg, unsigned int arglen, void *buf, unsigned int buflen)
{
    ...
    if (cmd == DVRFILE_IOCTL_GET_FD) {
        // Return the DVRP-side fd stored in privdata. This handle persists
        // across the IOP reboot (DVRP firmware keeps the file open).
        (void)arg;
        (void)arglen;
        (void)buf;
        (void)buflen;
