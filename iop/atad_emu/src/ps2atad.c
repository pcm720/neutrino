...
{
    M_DEBUG("%s(0x%x, %d, %d, %d, %d, %d, %d, %d, %d)\n", __FUNCTION__, buf, blkcount, feature, nsector, sector, lcyl, hcyl, select, command);
    return 0;
}
...
{
    M_DEBUG("%s(%d, 0x%x, %d, %d, %d)\n", __FUNCTION__, device, buf, lba, nsectors, dir);
