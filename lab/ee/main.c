 ...
 * - Vxxx/IOPRP.IMG — IOPRP firmware images in versioned folders (V260, V310, …)
...
//--------------------------------------------------------------
#define RUN_MEMMAP     1  // IOP heap layout scan (before sceCdInit)
#define RUN_MODULES    1  // module list + IOP free memory
...
#define RUN_SPEED      0  // read throughput: sceCdRead / fioRead / sceCdStRead
#define RUN_STREAM     0  // parameterized sceCdSt* sweep (mirrors IOP_RUN_STREAM)
#define RUN_IOP_MODULE 0  // load and run CDVD.IRX
...
{
    if (v == 0) return "[matches: bios,v1.40-v1.60]";
    if (v == 1) return "[matches: v1.65+]";
    return "[ERROR]";
}

//--------------------------------------------------------------
// IOPRP table — each entry maps a version label to the ISO9660 paths of the
// firmware image (and optional cdvdstm) on the test disc.
// One set per firmware version; where A/B cdvdstm variants exist, B is used
// (full sceCdReadCdda support).  Versioned folders: cdrom0:\\V<XYZ>\\...
// iop_path: passed to SifIopReset() — the IOP reads from disc itself.
//--------------------------------------------------------------
typedef struct {
    const char *name;          // label printed in output (e.g., "v2.60")
    const char *iop_path;      // SifIopReset path ("rom0:UDNL cdrom0:\\V260\\IOPRP.IMG;1")
    const char *cdvdstm_path;  // SifLoadModule path for cdvdstm, or NULL
} ioprp_entry_t;

static const ioprp_entry_t g_ioprp_table[] = {
    { "v1.40", "rom0:UDNL cdrom0:\\V140\\IOPRP.IMG;1", NULL },
    { "v1.46", "rom0:UDNL cdrom0:\\V146\\IOPRP.IMG;1", NULL },
    { "v1.50", "rom0:UDNL cdrom0:\\V150\\IOPRP.IMG;1", NULL },
    { "v1.60", "rom0:UDNL cdrom0:\\V160\\IOPRP.IMG;1", NULL },
    { "v1.65", "rom0:UDNL cdrom0:\\V165\\IOPRP.IMG;1", NULL },
    { "v2.02", "rom0:UDNL cdrom0:\\V202\\IOPRP.IMG;1", NULL },
    { "v2.05", "rom0:UDNL cdrom0:\\V205\\IOPRP.IMG;1", NULL },
    { "v2.10", "rom0:UDNL cdrom0:\\V210\\IOPRP.IMG;1", NULL },
    { "v2.11", "rom0:UDNL cdrom0:\\V211\\IOPRP.IMG;1", NULL },
    { "v2.13", "rom0:UDNL cdrom0:\\V213\\IOPRP.IMG;1", NULL },
    { "v2.14", "rom0:UDNL cdrom0:\\V214\\IOPRP.IMG;1", NULL },
    { "v2.20", "rom0:UDNL cdrom0:\\V220\\IOPRP.IMG;1", NULL },
    { "v2.21", "rom0:UDNL cdrom0:\\V221\\IOPRP.IMG;1", NULL },
    { "v2.24", "rom0:UDNL cdrom0:\\V224\\IOPRP.IMG;1", NULL },
    { "v2.30", "rom0:UDNL cdrom0:\\V230\\IOPRP.IMG;1", NULL },
    { "v2.34", "rom0:UDNL cdrom0:\\V234\\IOPRP.IMG;1", NULL },
    { "v2.41", "rom0:UDNL cdrom0:\\V241\\IOPRP.IMG;1", NULL },
    { "v2.42", "rom0:UDNL cdrom0:\\V242\\IOPRP.IMG;1", NULL },
    { "v2.43", "rom0:UDNL cdrom0:\\V243\\IOPRP.IMG;1", NULL },
    { "v2.50", "rom0:UDNL cdrom0:\\V250\\IOPRP.IMG;1", NULL },
    { "v2.52", "rom0:UDNL cdrom0:\\V252\\IOPRP.IMG;1", NULL },
    { "v2.53", "rom0:UDNL cdrom0:\\V253\\IOPRP.IMG;1", NULL },
    { "v2.55", "rom0:UDNL cdrom0:\\V255\\IOPRP.IMG;1", NULL },
    { "v2.60", "rom0:UDNL cdrom0:\\V260\\IOPRP.IMG;1", "cdrom0:\\V260\\CDVDSTM.IRX;1" },
    { "v2.70", "rom0:UDNL cdrom0:\\V270\\IOPRP.IMG;1", "cdrom0:\\V270\\CDVDSTM.IRX;1" },
    { "v2.71", "rom0:UDNL cdrom0:\\V271\\IOPRP.IMG;1", "cdrom0:\\V271\\CDVDSTM.IRX;1" },
    { "v2.80", "rom0:UDNL cdrom0:\\V280\\IOPRP.IMG;1", "cdrom0:\\V280\\CDVDSTM.IRX;1" },
    { "v3.00", "rom0:UDNL cdrom0:\\V300\\IOPRP.IMG;1", "cdrom0:\\V300\\CDVDSTM.IRX;1" },
    { "v3.10", "rom0:UDNL cdrom0:\\V310\\IOPRP.IMG;1", "cdrom0:\\V310\\CDVDSTM.IRX;1" },
    { NULL, NULL, NULL }
};
...
//--------------------------------------------------------------
// Section 0: IOP heap layout scan
// Loads MEMMAP.IRX which walks the sysmem allocator's internal block list
// and prints every block (used/free) to IOP TTY.  Run before sceCdInit to
// capture the post-IOPRP heap layout before game-side init allocates more.
//--------------------------------------------------------------
static void section_memmap(const char *label)
{
    int arg_len = strlen(label) + 1;
    int rv = SifLoadModule("cdrom0:\\MEMMAP.IRX;1", arg_len, label);
    _print("[MEMMAP_LOAD] %s -> %d\n", label, rv);
    // Give IOP time to finish printing (~50ms at 294MHz EE)
...
//--------------------------------------------------------------
// Section: Parameterized streaming — sweeps bufmax x bankmax x sectors-per-read
// Mirrors IOP_RUN_STREAM in iop/main.c.
//--------------------------------------------------------------

// Streaming parameter sweep constants (match IOP side)
...
// 1.5 MiB/s pacing: 2048 bytes / 1572864 bytes/s ≈ 1302 µs = 1302000 ns per sector
#define NS_PER_SECTOR 1302000L

#define STREAM_CB_MAX 128
static volatile int g_stream_cb_count;
static volatile int g_stream_reasons[STREAM_CB_MAX];

static void stream_callback(int reason)
{
...
static void __attribute__((unused)) section_stream(const sceCdlFILE *fp)
{
    sceCdRMode mode = { .trycount=0, .spindlctrl=SCECdSpinNom,
                        .datapattern=SCECdSecS2048, .pad=0 };

    sceCdCallback(stream_callback);
    _print("[STREAM_PARAM_TEST]\n");

    for (int bi = 0; bi < N_BUFMAX; bi++) {
        int bufmax = k_bufmax[bi];
        // Ring buffer lives on IOP heap — sceCdStInit takes an IOP-side pointer
        void *iop_buf = SifAllocIopHeap((u32)bufmax * 2048);
        if (!iop_buf) {
            _print("  [B=%d] iop_alloc_failed\n", bufmax);
            continue;
        }

        for (int ki = 0; ki < N_BANKMAX; ki++) {
            int bankmax = k_bankmax[ki];
            int init_ret = sceCdStInit((u32)bufmax, (u32)bankmax, iop_buf);
            ...
            for (int si = 0; si < N_SECTORS; si++) {
                ...
                for (int r = 0; r < STREAM_READS; r++) {
                    // STMNBLK (0): non-blocking read — returns sectors available
                    int rd = sceCdStRead(nsec, (u32 *)g_buf2, 0, &st_err);
                    if (rd <= 0) { stream_ok = 0; break; }
                    // Pace at ~1.5 MiB/s between reads
                    struct timespec tvp = { .tv_sec = 0, .tv_nsec = (long)nsec * NS_PER_SECTOR };
                    nanosleep(&tvp, NULL);
                }
                ...
                _print("  [B=%d K=%d S=%d] init=%d start=%d stop=%d ok=%d err=%u cb=%d r:",
                       bufmax, bankmax, nsec, init_ret, start_ret, stop_ret,
                       stream_ok, st_err, g_stream_cb_count);
                for (int i = 0; i < n; i++) _print(" %d", g_stream_reasons[i]);
                _print("\n");
            }
        }
...
// iop_path: cdrom0: path for SifIopReset (e.g. "cdrom0:\\V260\\IOPRP.IMG;1"),
//           or NULL for plain BIOS baseline.
// is_subsequent: 1 if SifLoadFileExit/ExitIopHeap need to be called first.
//--------------------------------------------------------------
static void run_tests(const char *label, const char *iop_path, const char *cdvdstm_path)
{
    ...
    if (iop_path != NULL) {
        ...
        if (cdvdstm_path != NULL) {
            int ret = SifLoadModule(cdvdstm_path, 0, NULL);
            _print("  [cdvdstm] %s -> %d\n", cdvdstm_path, ret);
        }
    }

#if RUN_MEMMAP
    section_memmap(label);
#endif
...
int main()
{
    ...
    //run_tests("bios_baseline", NULL, NULL);

    // Each IOPRP version — SifIopReset loads directly from disc
    for (int i = 0; g_ioprp_table[i].name != NULL; i++)
        run_tests(g_ioprp_table[i].name, g_ioprp_table[i].iop_path, g_ioprp_table[i].cdvdstm_path);
