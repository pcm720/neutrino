...
| Run 10 | 2026-03-25 | PCSX2 v2.5.336 | Custom test ISO (CD, 512 KB TEST.BIN) | IOP only — parameterized streaming sweep: B=32 × K=4 × S={1,2,4,8,16,32}; all 22 versions (renamed v1.40–v3.10); cdvdstm.irx loaded externally for v2.60+ |
| Run 11 | 2026-03-25 | PCSX2 v2.5.336 | Custom test ISO (CD, 512 KB TEST.BIN) | Same as Run 10 with PCSX2 CDVD sector-read debug logging enabled; CDRead block sizes, sector sizes, and re-read patterns captured for all 29 firmware versions |
...
## IOP Streaming Table (Run 10 — cdvdstm variants included)

`sceCdStInit(32,4,buf)` + `sceCdStStart(lsn,&mode)` + 200ms wait + 4× `sceCdStRead(4,...,0,&err)`
+ `sceCdStStop()`. Callback registered across the entire sequence with all reasons recorded.
cdvdstm.irx loaded externally for v2.60+. Loop condition updated from `rd != 1` (Run 7) to
`rd <= 0`, so Era 3 variants now complete all 4 reads instead of breaking after the first.

| Version | init | start | cb_count | reasons (in order) |
|---|:---:|:---:|:---:|:---|
| v1.40 | 1 | 1 | 7 | `1 1 1 1 1 1 1`† |
| v1.46 | 1 | 1 | 8 | `1 1 1 1 1 1 1 1`† |
| v1.50 | 1 | 1 | 7 | `1 1 1 1 1 1 1` |
| v1.60 | 1 | 1 | 7 | `1 1 1 1 1 1 1` |
| v1.65 | 1 | 1 | 7 | `1 1 1 1 1 1 1` |
| **← sceCdStStop fires reason=8 →** | | | | |
| v2.02 | 1 | 1 | 8 | `1 1 1 1 1 1 1 8` |
| v2.05 | 1 | 1 | 8 | `1 1 1 1 1 1 1 8` |
| v2.10 | 1 | 1 | 8 | `1 1 1 1 1 1 1 8` |
| v2.11 | 1 | 1 | 8 | `1 1 1 1 1 1 1 8` |
| v2.13 | 1 | 1 | 8 | `1 1 1 1 1 1 1 8` |
| v2.14 | 1 | 1 | 8 | `1 1 1 1 1 1 1 8` |
| v2.20 | 1 | 1 | 8 | `1 1 1 1 1 1 1 8` |
| v2.21 | 1 | 1 | 8 | `1 1 1 1 1 1 1 8` |
| v2.24 | 1 | 1 | 8 | `1 1 1 1 1 1 1 8` |
| v2.30 | 1 | 1 | 8 | `1 1 1 1 1 1 1 8` |
| **← cb_count drops; reason=8 removed →** | | | | |
| v2.34 | 1 | 1 | 5 | `1 1 1 1 1` |
| v2.41 | 1 | 1 | 5 | `1 1 1 1 1` |
| v2.42 | 1 | 1 | 5 | `1 1 1 1 1` |
| v2.43 | 1 | 1 | 5 | `1 1 1 1 1` |
| v2.50 | 1 | 1 | 5 | `1 1 1 1 1` |
| v2.52 | 1 | 1 | 5 | `1 1 1 1 1` |
| v2.53 | 1 | 1 | 5 | `1 1 1 1 1` |
| v2.55 | 1 | 1 | 5 | `1 1 1 1 1` |
| **← sceCdSt* in cdvdstm.irx; consumption-based callbacks; ok=1 for all S →** | | | | |
| v2.60 | 1 | 1 | **2** | `1 1` |
| v2.70 | 1 | 1 | **2** | `1 1` |
| v2.71 | 1 | 1 | **2** | `1 1` |
| v2.80 | 1 | 1 | **2** | `1 1` |
...
**Column notes:**
- `init`: `sceCdStInit(32, 4, buf)` return — 1=ok
- `start`: `sceCdStStart(lsn, &mode)` return — 1=ok
- `cb_count`: total callback invocations during the streaming session
- `reasons`: ordered list — 1=`SCECdFuncRead`, 8=`SCECdFuncBreak`
- For v2.60+: cdvdstm.irx loaded externally; without it, cdvdman stubs return 0 for all sceCdSt* calls (as shown in Run 7)

† v1.40/v1.46: timer-driven callbacks — count grows with test duration and may vary between runs.

**Era 3 change vs Run 7:** Run 7 used `if (rd != 1) break`, which stopped after the first
`sceCdStRead` call (since it returns the sector count 4, not 1). That gave cb_count=4 = bankmax.
Run 10 uses `if (rd <= 0) break`, completing all 4 reads. The extra callback (cb=5 vs 4) reflects
one bank refill that fires while the reads are in progress.

**cdvdstm callback model:** Unlike cdvdman, cdvdstm fires one callback per bank-boundary
*crossed by the reader* rather than per bank filled by the drive. For B=32, K=4 (bank_size=8):
4 reads × 4 sectors = 16 sectors = 2 bank-boundaries → cb=2. This is consumption-based vs
the fill-based model of cdvdman.
...
## IOP Streaming Parameter Sweep (Run 10 — all versions, B=32 K=4)

`bufmax=32`, `bankmax=4`, `sectors-per-read` ∈ {1,2,4,8,16,32}. `STREAM_READS=4`, 200ms
pre-fill, pacing `nsec × 1302 µs` between reads. cdvdstm.irx loaded for v2.60+.
Loop stops when `rd ≤ 0` (updated from Run 8's `rd ≠ 1`).

`sceCdStInit(32,4,buf)` prints: `Stream Buffer 1Bank 16384byte 4banks … 65536byte used`
→ bank_size=8 sectors, 4 banks, ring=32 sectors total.

### cb_count vs sectors-per-read (B=32, K=4)

| Version | S=1 | S=2 | S=4 | S=8 | S=16 | S=32 | ok=1 for S≤ |
|---|:---:|:---:|:---:|:---:|:----:|:----:|:-----------:|
| v1.40 | 7† | 7† | 7† | 10† | 14† | 25† | all |
| v1.46 | 8† | 7† | 8† | 10† | 15† | 26† | all |
| v1.50 | 8 | 7 | 7 | 8 | 8 | 8 | 8 |
| v1.60 | 8 | 7 | 7 | 8 | 8 | 8 | 8 |
| v1.65 | 8 | 7 | 7 | 8 | 8 | 8 | 8 |
| v2.02 | 9 | 8 | 8 | 8 | 9 | 9 | 4 |
...
| v2.24 | 9 | 8 | 8 | 8 | 9 | 9 | 4 |
| v2.30 | 9 | 8 | 8 | 8 | 9 | 9 | 4 |
| v2.34 | 4 | 4 | 5 | 7 | 8 | 5 | 16 |
| v2.41 | 4 | 4 | 5 | 7 | 8 | 5 | 16 |
| v2.42 | 4 | 4 | 5 | 7 | 8 | 5 | 16 |
| v2.43 | 4 | 4 | 5 | 7 | 8 | 5 | 16 |
| v2.50 | 4 | 4 | 5 | 7 | 8 | 5 | 16 |
| v2.52 | 4 | 4 | 5 | 7 | 8 | 5 | 16 |
| v2.53 | 4 | 4 | 5 | 7 | 8 | 5 | 16 |
| v2.55 | 4 | 4 | 5 | 7 | 8 | 5 | 16 |
| **v2.60 (cdvdstm)** | **1** | **1** | **2** | **4** | **5** | **5** | **all** |
| **v2.70 (cdvdstm)** | **1** | **1** | **2** | **4** | **5** | **5** | **all** |
| **v2.71 (cdvdstm)** | **1** | **1** | **2** | **4** | **5** | **5** | **all** |
| **v2.80 (cdvdstm)** | **1** | **1** | **2** | **4** | **5** | **5** | **all** |
| **v3.00 (cdvdstm)** | **1** | **1** | **2** | **4** | **5** | **5** | **all** |
| **v3.10 (cdvdstm)** | **1** | **1** | **2** | **4** | **5** | **5** | **all** |

† Era 0 (v1.40/v1.46): timer-driven; cb_count grows with test duration (larger S = longer pacing delay = more bank fills). Values may vary between runs.

`ok=1 for S≤` column: the largest S value at which all STREAM_READS=4 reads return `rd > 0`. Beyond this S, the ring buffer is drained faster than the drive refills it and at least one read returns 0 (`ok=0`).

### Callback model by era

| Era | Versions | cb model | reason=8 |
|-----|----------|----------|----------|
| 0 | v1.40–v1.46 | timer-driven; grows with duration | No |
| 1 | v1.50–v1.65 | fill-based; ≈7–8 callbacks, capped | No |
| 2 | v2.02–v2.30 | fill-based; ≈8–9 callbacks, capped | Yes (last callback = sceCdStStop) |
| 3 | v2.34–v2.55 | mixed; cb=bankmax for small S, grows for large S | No |
| 4 | v2.60+ (cdvdstm) | consumption-based; cb = min(⌈4S/8⌉, bankmax+1) | No |

**Era 4 formula:** for B=32, K=4 (bank_size=8): `cb = min(ceil(STREAM_READS × S / bank_size), bankmax + 1)`
= `min(ceil(S/2), 5)`. One callback fires per bank-boundary crossed by the reader; the
+1 cap reflects one final refill after the last bank is consumed before `sceCdStStop`.

**Era 4 vs Run 8:** Run 8 found "cb_count independent of S for ioprp165+" — that was an
artifact of the old `rd ≠ 1` loop break: Era 3 firmware returns `rd = nsec` (sector count),
which broke the loop after one read, leaving only pre-fill callbacks (= bankmax, constant).
With the corrected `rd ≤ 0` condition, Era 3 shows S-dependent cb_count. Era 4 (cdvdstm) was
not tested in Run 8 (cdvdstm was not loaded); it is natively S-dependent.
...
## Finding 10: CDRead Sector Patterns by Era (Run 11)

Run 11 captured PCSX2's low-level CDRead log alongside the streaming test output. Every
CDRead issued by the CDVD driver is visible — both the sector address, block count, and
reported sector byte size. All 29 firmware versions were tested with B=32, K=4 (bank=8 sectors).

### CDRead summary by era

| Era | Versions | CDRead blk | Sector sz | Re-reads on ring-full |
|-----|----------|:----------:|:---------:|:---------------------:|
| 0A | v1.40 | **16** | 2048 | **Yes** (22 re-reads) |
| 0B | v1.46 | 8 | 2048 | No |
| 1 | v1.50–v1.65 | 8 | 2048 | No |
| 2 | v2.02–v2.30 | 8 | 2048 | No |
| 3 | v2.34–v2.55 | 8 | 2048 | No |
| 4 | v2.60–v3.10 | 8 | **2340** | No |

### Era 0A (v1.40): 16-sector CDRead, timer fires unconditionally

The v1.40 cdvdman issues CDReads in **16-sector (2-bank) chunks** regardless of bank
geometry. When the ring buffer is full, the timer continues to fire and re-issues the
CDRead from the **same sector position**, overwriting previously fetched data. The drive
advances only after the consumer drains at least one bank. 22 re-reads were observed
during the B=32 K=4 S=1 sweep.

Callback behaviour: one callback fires per timer tick regardless of ring state. The
callback count grows unboundedly with test duration (`cb` grows with `S`):

```
S=1:  cb=7   S=4:  cb=7   S=16: cb=14  S=32: cb=25
```

The re-read is a direct consequence of the tick-driven model: the timer does not inspect
whether the destination bank slot is already valid before issuing the next CDRead.

### Era 0B (v1.46): 8-sector CDRead, timer-driven, no underflow

v1.46 switched to **8-sector (1-bank) CDReads** and eliminated re-reads: when the ring is
full the timer fires a callback but issues no CDRead. The callback count still grows
unboundedly with test duration — v1.46 is not capped:

```
S=1: cb=8   S=4: cb=8   S=16: cb=15   S=32: cb=26
```

All S values succeed (`ok=1`). The timer fires often enough to keep up with even the
fastest consumer reads tested here.

### Era 1 (v1.50–v1.65): timer capped at 8 callbacks; ring underflow for large S

Starting at v1.50, the callback count **caps at 8** regardless of how much data the
consumer reads:

```
S=1:  cb=8   S=4:  cb=7   S=8:  cb=8   S=16: cb=8   S=32: cb=8
```

With only 8 banks pre-filled (8 × 8 = 64 sectors) and no further refills, large consumer
reads exhaust the ring:

- S≤4 (16 sectors consumed): ring holds ~64 sectors → no underflow → `ok=1`
- S=16 (64 sectors consumed): ring just empties → `ok=0` (4th read fails)
- S=32 (128 sectors consumed): ring empties after 2 reads → `ok=0`

The timer stops after 8 firings; no demand-driven refill exists yet.

### Era 2 (v2.02–v2.30): same as Era 1, plus reason=8 on stop

Callback counts and underflow behaviour are identical to Era 1 (cap at 8–9 streaming
callbacks). The only addition is the `sceCdStStop()` callback delivering `reason=8`
(`SCECdFuncBreak`) as the final event, explaining the `cb=9` vs `cb=8` split observed in
Run 10:

```
v2.02 S=1: cb=9 r: 1 1 1 1 1 1 1 1 8   (8 streaming + 1 stop)
v2.02 S=8: cb=8 r: 1 1 1 1 1 1 1 8     (7 streaming + 1 stop, ok=0)
```

Ring underflow now also appears at S=8 for some sub-versions, suggesting the cap
tightened to 7 actual streaming callbacks (before the stop callback). Consistent with
Finding 9's explanation that `sceCdStStop()` calls `sceCdBreak` to abort the in-flight
background CDRead, which fires reason=8.

### Era 3 (v2.34–v2.55): demand-driven, 1 callback per CDRead

CDRead size stays at 8 sectors. The timer is gone: each CDRead completion fires exactly
one callback, and a new CDRead is issued per bank consumed. The callback count now
follows consumption:

```
S=1: cb=4   S=2: cb=4   S=4: cb=5   S=8: cb=7   S=16: cb=8   S=32: cb=5
```

With B=32, K=4, bank=8: pre-fill issues exactly `bankmax=4` CDReads → 4 callbacks. Each
additional bank consumed issues 1 more CDRead → 1 more callback. `ok=1` for all S values
— demand-driven refills keep up with any consumer rate.

The observed CDRead positions for S=1 confirm clean sequential fills with no re-reads:
```
Sector 394 (+0)   bank 0 → cb fires
Sector 402 (+8)   bank 1 → cb fires
Sector 410 (+16)  bank 2 → cb fires
Sector 418 (+24)  bank 3 → cb fires   ← ring full; no further CDReads for S=1
```

### Era 4 (v2.60–v3.10 / cdvdstm): pre-fill silent, ring-full fires 1 callback

cdvdstm issues CDReads in **8-sector chunks** but uses a **raw sector size of 2340 bytes**
(vs 2048 for all earlier versions). 2340 = full CD sector minus the 12-byte sync pattern
(header + subheader + data + EDC + ECC). cdvdstm reads the raw sub-channel format and
strips/processes it internally.

The callback model is restructured: callbacks do **not** fire during pre-fill. One callback
fires when the ring first becomes **completely full** (all `bankmax` banks loaded). Subsequent
callbacks fire one-per-bank as each refill CDRead completes:

```
S=1: cb=1   S=2: cb=1   S=4: cb=2   S=8: cb=4   S=16: cb=5   S=32: cb=5
```

For S=1: pre-fill 4 CDReads → ring full → `cb=1`. Consumer reads 1 sector = ⅛ bank →
no refill triggered → total `cb=1`.

For S=16: pre-fill → `cb=1`; consumer reads 64 sectors = 8 banks consumed; only 4 refill
callbacks complete within STREAM_READS=4 loop → `cb=5`.

The CDRead positions for S=32 are the longest sequence (19 CDReads observed):
```
Sector 394 (8 blks, sz=2340)  ← bank 0  (no cb)
Sector 402 (8 blks, sz=2340)  ← bank 1  (no cb)
Sector 410 (8 blks, sz=2340)  ← bank 2  (no cb)
Sector 418 (8 blks, sz=2340)  ← bank 3  → ring full → cb=1 fires
Sector 426 (8 blks, sz=2340)  ← refill  → cb=2 fires
Sector 434 (8 blks, sz=2340)  ← refill  → cb=3 fires
...
```

All cdvdstm versions (v2.60–v3.10) produce **identical** CDRead patterns and callback counts.

### cb(S) table for B=32 K=4 — all versions

| Version | S=1 | S=2 | S=4 | S=8 | S=16 | S=32 | ok for large S |
|---------|----:|----:|----:|----:|-----:|-----:|:--------------:|
| v1.40 | 7 | 7 | 7 | 10 | 14 | 25 | ✓ (timer keeps up) |
| v1.46 | 8 | 7 | 8 | 10 | 15 | 26 | ✓ (timer keeps up) |
| v1.50 | 8 | 7 | 7 | 8 | 8 | 8 | ✗ S≥16 |
| v1.60 | 8 | 7 | 7 | 8 | 8 | 8 | ✗ S≥16 |
| v1.65 | 8 | 7 | 7 | 8 | 8 | 8 | ✗ S≥16 |
| v2.02 | 9 | 8 | 8 | 8 | 9 | 9 | ✗ S≥8 |
| v2.05–v2.21 | 9 | 8 | 8 | 9 | 9 | 9 | ✗ S≥16 |
| v2.24–v2.30 | 9 | 8 | 8 | 8 | 9 | 9 | ✗ S≥16 |
| v2.34–v2.55 | 4 | 4 | 5 | 7 | 8 | 5 | ✓ (demand-driven) |
| v2.60–v3.10 | 1 | 1 | 2 | 4 | 5 | 5 | ✓ (cdvdstm) |

Note: for Era 2, the last callback in each run is always reason=8; counts shown include it.

### Neutrino implications

**Sector size 2340**: `cdvdman_emu` streaming (invoked via cdvdstm.irx) must request raw
sub-channel sectors (CDVD raw mode) and strip the header/subheader/EDC/ECC bytes before
writing data into the ring buffer. If the underlying read delivers 2048-byte logical sectors
instead, the per-sector byte offset seen by the game will be wrong.

**Timer-based era underflow**: Eras 1–2 (v1.50–v2.30) games that use large `S` values
per `sceCdStRead` call may stall if Neutrino's streaming doesn't replicate the timer
behaviour. However, those games were written for firmware that also exhibited the underflow,
so they should not request more sectors than the ring holds in a single call.

**Era 0B (v1.46) vs Era 1 (v1.50)**: v1.46 has an uncapped timer; v1.50+ cap at 8. This
is the only version-pair boundary within the "timer-driven" group where application-visible
behaviour (whether S=32 succeeds) differs. Games targeting v1.46 hardware are unlikely to
exist in practice, but the boundary is documented here.
